...
interface SearchClientNameProps {
  onDeleteSuccess?: () => void;
}

export function SearchClientName({ onDeleteSuccess }: SearchClientNameProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [searchResults, setSearchResults] = useState<VinRecord[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [notFound, setNotFound] = useState(false);
  const [showAddPartModal, setShowAddPartModal] = useState<string | null>(null);
  const [showRemovePartModal, setShowRemovePartModal] = useState<string | null>(null);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    if (!searchQuery.trim()) {
      setError('Introdu un nume de client');
      return;
    }
    ...
    try {
      const { data, error: searchError } = await supabase
        .from('vin_records')
        .select('*')
        .ilike('client_name', `%${searchQuery.trim()}%`);
          ...
          >
            <div className="text-center">
              <p className="text-lg font-semibold text-brand-charcoal">
                S-au gasit <span className="text-brand-red">{searchResults.length}</span> rezultat{searchResults.length !== 1 ? 'e' : ''}
              </p>
            </div>
                ...
                <Card variant="light" className="border-4 border-brand-charcoal overflow-hidden">
                  ...
                  <div className="bg-gradient-to-r from-brand-charcoal to-brand-charcoal-light text-white px-6 py-5 -m-6 mb-6">
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                      <CardTitle className="text-white text-xl">Detalii Client</CardTitle>
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      >
                        ...
                        <p className="text-lg font-bold text-brand-charcoal">{result.client_name}</p>
                      </motion.div>
                      ...
                      >
                        ...
                        <p className="text-lg font-mono font-bold text-brand-charcoal">{result.vin_number}</p>
                      </motion.div>
                    ...
                    <div className="pt-4 border-t-2 border-gray-200">
                      <p className="text-xs text-gray-500 font-medium">
                        Creat: {result.created_at ? new Date(result.created_at).toLocaleString('ro-RO') : 'N/A'}
                      </p>
                    </div>
                  </CardContent>
                </Card>
